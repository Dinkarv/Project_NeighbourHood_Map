...
*/
  for (var i = 0; i < locations.length; i++) {
    ...
    marker.addListener('click', function() {
      var marker = this;
      viewmodel.getWikiInfo(marker, infowindow);
...
function makeMarkerIcon(markerColor) {
  var markerImage = new google.maps.MarkerImage('http://chart.googleapis.com/chart?chst=d_map_spin&chld=1.15|0|' + markerColor +
    '|40|_|%E2%80%A2', new google.maps.Size(21, 34), new google.maps.Point(0, 0), new google.maps.Point(10, 34), new google.maps.Size(21, 34));
  return markerImage;
}
...
var viewmodel = {
  myFaveCafes: ko.observableArray(locations),
  ...
  /*go_places: function () {
        ...
        placesService.textSearch({
            query: document.getElementById('places-search')
                .value
            , bounds: bounds
...
*/
  ...
  fltr_name: function(cafe, target) {
    //console.log(cafe, target);
    var marker = cafe.marker;
    var click = 0;
    var largeInfo = new google.maps.InfoWindow();

     viewmodel.getWikiInfo(marker,largeInfo);
  ...
  filter: function(value) {
    ...
    //to perform filter matching purposes..
...
viewmodel.addMarkers = function() {
    viewmodel.myFaveCafes().forEach(function(cafe,i) {
      cafe.marker = markers[i];

  });
};

viewmodel.q.subscribe(viewmodel.filter);
